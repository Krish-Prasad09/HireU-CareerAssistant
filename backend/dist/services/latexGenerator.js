...
function profileId(value, domain) {
...
export function generateLatexResume(data) {
    ...
    const educationRows = (data.education ?? [])
        .filter((edu) => hasAnyValue([edu.degree, edu.institute, edu.cgpaOrPercentage, edu.year]))
        .map((edu) => `  ${escapeLatex(edu.degree)} & ${escapeLatex(edu.institute)} & ${escapeLatex(edu.cgpaOrPercentage)} & ${escapeLatex(edu.year)} \\\\\\hline`)
        .join("\n");
    const experienceContent = (data.experience ?? [])
        .filter((exp) => hasAnyValue([exp.organization, exp.location, exp.role, exp.dates]))
        .map((exp) => {
...
${bullets}
    ...
    const projectsContent = (data.projects ?? [])
        .filter((project) => hasAnyValue([project.name, project.subtitle, project.dates, project.linkLabel, project.linkUrl]))
        .map((project) => {
...
${bullets}
    ...
    const skillsContent = (data.skills ?? [])
        ...
        .map((skill) => `  \\resumeSubItem{${escapeLatex(skill.category)}}
    {${escapeLatex(skill.items)}}`)
        .join("\n");
    const coursesContent = (data.courses ?? [])
        .filter((course) => hasAnyValue([course.category, course.items]))
        .map((course) => `  \\resumeSubItem{${escapeLatex(course.category)}}
    {${escapeLatex(course.items)}}`)
        .join("\n");
    const positionsContent = (data.positions ?? [])
        .filter((position) => hasAnyValue([position.position, position.organization, position.tenure]))
        .map((position) => `  \\resumePOR{${escapeLatex(position.position)}}
    {${escapeLatex(position.organization)}}
    {${escapeLatex(position.tenure)}}`)
        .join("\n");
    const achievementsContent = (data.achievements ?? [])
        .filter((achievement) => hasAnyValue([achievement.title, achievement.description, achievement.year]))
        .map((achievement) => `  \\resumePOR{${escapeLatex(achievement.title)}}
