...
// Job board apply links — search by title
function buildApplyLinks(title: string) {
  ...
  return [
    ...
    {
      name: "Internshala",
      url: `https://internshala.com/jobs/keywords-${q}`,
    ...
    {
      name: "Glassdoor",
      url: `https://www.glassdoor.co.in/Job/india-${title
        .toLowerCase()
        .replace(/\s+/g, "-")}-jobs-SRCH_IL.0,5_IN115_KO6,${
        6 + title.length
      }.htm`,
...
const JobMatcherPage = () => {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (error?.response?.data?.showPayment) {
  ...
  return (
    <>
      {showCreditGate && (
        <CreditGate onClose={() => setShowCreditGate(false)} />
      )}
              ...
              {result.jobs.map((job, i) => (
                ...
                >
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {buildApplyLinks(job.title).map((link) => (
                        <a
                          key={link.name}
                          href={link.url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className={`feature-pill gap-1.5 border ${link.color} hover:opacity-80 transition-opacity`}
                        >
