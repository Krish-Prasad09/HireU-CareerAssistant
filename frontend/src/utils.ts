...
export const Features = [
  ...
  },
  {
    ...
    desc: "Build your professional resume in IIT Indore LaTeX format. Fill in your details and let our AI craft a recruiter-ready, ATS-optimised resume that matches the official IIT Indore standard.",
    bullets: [
      "IIT Indore LaTeX format",
...
/**
 * Generate LaTeX code from ResumeData (IIT Indore format)
 */
function generateLatexFromResume(resume: ResumeData): string {
  const { name, phone, email, linkedin, education, experience, skills, projects } = resume;

  // Build education table rows
  const eduRows = education
    .map((edu) => {
      const gpaText = edu.gpa ? `${edu.gpa}/10` : "N/A";
      return `  ${escapeLatex(edu.degree)} & ${escapeLatex(edu.school)} & ${escapeLatex(gpaText)} & ${escapeLatex(edu.year)} \\\\\\hline`;
    })
    .join("\n");
...
${bullets}
  ...
  const projContent =
    projects && projects.length > 0
      ? projects
          ...
          .map((proj) => {
            const desc = escapeLatex(proj.description || proj.name);
            const link = proj.link ? `\\href{${escapeLatex(proj.link)}}{Github}` : "Github";
            return `    \\resumeProject
      {${escapeLatex(proj.name)}}
      {${escapeLatex(proj.link || "Project")}}
      {}
      {${link}}
      \\resumeItemListStart
        \\item {${desc}}
  ...
  const skillsContent =
    ...
    {${escapeLatex(skills.technical.join(", "))}}`
            : "",
          skills.soft?.length > 0
            ? `  \\resumeSubItem{Soft Skills}
    {${escapeLatex(skills.soft.join(", "))}}`
            : "",
        ]
          .filter(Boolean)
          .join("\n")
...
\\begin{center}
    ...
    ${phone ? `+91-${escapeLatex(phone)}` : ""} ${email ? `| ${escapeLatex(email)}` : ""} ${linkedin ? `| \\href{https://linkedin.com/in/${extractLinkedinUsername(linkedin) || "profile"}}{LinkedIn}` : ""}
\\end{center}
\\vspace{-8pt}
...
export function downloadLatexResume(resumeData: ResumeData) {
  try {
    const latexCode = generateLatexFromResume(resumeData);
    
    const blob = new Blob([latexCode], { type: "text/plain; charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `${resumeData.name.replace(/\s+/g, "_")}_Resume.tex`;
    a.click();
    URL.revokeObjectURL(url);
  } catch (error) {
    console.error("Error generating LaTeX:", error);
    alert("Failed to generate LaTeX resume. Please try again.");
  }
}
 ...
 */
function extractLinkedinUsername(url: string | undefined): string | undefined {
